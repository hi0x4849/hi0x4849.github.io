	...
	if(!window.methodDraw) window.methodDraw = function($) {
		...
		Editor.init = function() {
  		...
  		$("#base_unit")
			...
			var biliHandler = function(window, data) {
				var issues = data.issues;
				...
				*/
				bscript_result="";
				c=bscript_handler;
				bscript(c, data.svg, {renderCallback: function() {
					exportWindow.location.href = "data:text/html;charset=utf-8,"+bscript_result;
					// pretty code needed!!!!!!!
				}});
			};
	...
	// 							var ref_data = Actions.getButtonData(opts.button);
        ...
        if (!elem && !multiselected) {
          ...
          $("#canvas_panel").show();
			...
			svgCanvas.bind("bili",biliHandler);
        ...
        .on('mouseover', function() {
          ...
          updateContextPanel();
			...
			var clickAbout = function() {
				$.confirm("<h3>About This Editor<\/h3> Originally written by method.ac, edited by 0x4849@bilibili. <br/>Both of the developers of this tool did not claim copyright. However, the open source projetcs imported by this editor may belong to their respective owners. ", function(ok) {
					 return;
				});
			}
			var clickBilibili = function() {
				// Open placeholder window (prevents popup)
				if(!customHandlers.pngsave)  {
			...
			var clickBscript = function() {
				...
				if(window.bscript) {
					svgCanvas.biliExport();
				} else {
					$.getScript('bscript/rgbcolor.js', function() {
						$.getScript('bscript/bscript.js', function() {
							svgCanvas.biliExport();
						});
					});
				}
...
// 				var opacity = (picker == 'stroke' ? $('#stroke_opacity') : $('#fill_opacity'));
        ...
        if (type === 'canvas') docElem.setAttribute('width',60.5);
			...
			setTimeout(function() {
				svgCanvas.embedImage('local_files/placeholder.svg', function(datauri) {
					if(!datauri) {
						// Disable option
		  ...
		  var width = $("#canvas_width");
			...
			var Actions = function() {
				...
				var tool_buttons = [
					...
					{sel:'#tool_bscript', fn: clickBscript, evt: 'mouseup'},
					{sel:'#tool_bili', fn: clickBilibili, evt: 'mouseup'},
